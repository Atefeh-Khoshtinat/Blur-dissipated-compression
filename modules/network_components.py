...
class Block(nn.Module):
    def __init__(self, dim, dim_out, large=False):
        ...
        self.block = nn.Sequential(
            nn.Conv2d(dim, dim_out, 7 if large else 3, padding=3 if large else 1),
            LayerNorm(dim_out),
...
class ResnetBlock(nn.Module):
    ...
    def forward(self, x, time_emb=None):
        h = self.block1(x)
        if exists(time_emb):
            time_proj = self.mlp(time_emb)[:, :, None, None]
            if time_proj.shape[1] != h.shape[1]:
                time_proj = self.mlp(time_emb).unsqueeze(-1).unsqueeze(-1)  # [B, C, 1, 1]
